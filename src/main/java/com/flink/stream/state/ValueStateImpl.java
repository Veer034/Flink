...
public class ValueStateImpl {
    public static void main(String[] args) throws Exception {
        ...
        String output = "file:///Users/ranvsing/CodeBase/Flink/output/state/valuestate";
        ...
        DataStream<Long> sum = data.map(new MapFunction<String, Tuple2<Long, Long>>() {
        ...
        // execute program
        environment.execute("Value State implementation.");
    }
     ...
     */
    public static class StatefulMap extends RichFlatMapFunction<Tuple2<Long, Long>, Long> {
         ...
         */
        public void flatMap(Tuple2<Long, Long> input, Collector<Long> out) throws Exception {
            Long currCount = count.value();
            currCount += 1;
            count.update(currCount);
            sum.update(sum.value() + input.f1);
            if (currCount >= 10) {
