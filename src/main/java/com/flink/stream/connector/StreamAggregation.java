...
public class StreamAggregation
{
    public static void main(String[] args) throws Exception
    {
         ...
         * */
        mapped.keyBy(0).sum(2).writeAsText(params.get("output")+"/agg-sum",FileSystem.WriteMode.OVERWRITE);;
        mapped.keyBy(0).min(2).writeAsText(params.get("output")+"/agg-min",FileSystem.WriteMode.OVERWRITE);;
        mapped.keyBy(0).minBy(2).writeAsText(params.get("output")+"/agg-minby",FileSystem.WriteMode.OVERWRITE);;
        mapped.keyBy(0).max(2).writeAsText(params.get("output")+"/agg-max",FileSystem.WriteMode.OVERWRITE);;
        mapped.keyBy(0).maxBy(2).writeAsText(params.get("output")+"/agg-maxBy",FileSystem.WriteMode.OVERWRITE);;
        // execute program
